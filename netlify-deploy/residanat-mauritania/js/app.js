...
const setupPWA = () => { if ('serviceWorker' in navigator) { caches.keys().then(cacheNames => { cacheNames.forEach(cacheName => { if (cacheName.startsWith('residanat-nktt')) { caches.delete(cacheName); console.log('Cleared old cache:', cacheName); } }); }); navigator.serviceWorker.register('./sw.js?v=9').then(registration => { console.log('Service Worker registered:', registration); registration.update(); }).catch(error => console.error('Service Worker registration failed:', error)); } };
