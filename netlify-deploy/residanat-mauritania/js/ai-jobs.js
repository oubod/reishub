(function (root) {
  ...
  let user, jobs = [], timer, poller, channel, refreshPending;
  ...
  async function refresh() {
    ...
    refreshPending = (async () => { const { data, error } = await supabaseClient.from("mauritania_ai_jobs").select("*").order("created_at", { ascending: false }); if (error) throw error; jobs = data || []; renderJobs(); renderLibrary(); })();
  ...
  function clockText(job) { const elapsed = seconds(job), estimate = job.estimated_seconds || job.question_count * 10, remaining = Math.max(0, Math.round(estimate * (1 - (job.progress || 0) / 100))); return `Écoulé ${duration(elapsed)}${ACTIVE.includes(job.status) ? ` · restant ≈ ${duration(remaining)}` : ""}`; }
  function tick() { document.querySelectorAll(".ai-job[data-job]").forEach((card) => { const job = jobs.find((item) => item.id === card.dataset.job), clock = card.querySelector(".ai-clock"); if (job && clock) clock.textContent = clockText(job); }); }
  function jobCard(job) {
    const active = ACTIVE.includes(job.status);
    return `<article class="ai-job" data-job="${h(job.id)}"><div class="ai-job-head"><div><span class="ai-status ${h(job.status)}">${LABELS[job.status] || h(job.status)}</span><h3>${h(job.title || job.source_name)}</h3><p>${job.question_count} questions · ${h(job.question_type?.toUpperCase())} · ${h(job.specialty || "Toutes spécialités")}</p></div><strong>${job.progress || 0} %</strong></div><div class="ai-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${job.progress || 0}"><span style="width:${job.progress || 0}%"></span></div><div class="ai-job-meta"><span>Étape : ${h(job.stage || LABELS[job.status])}</span><span class="ai-clock">${clockText(job)}</span>${job.input_tokens || job.output_tokens ? `<span>${job.input_tokens || 0} jetons d’entrée · ${job.output_tokens || 0} de sortie</span>` : ""}</div>${job.status === "queued" ? `<p class="ai-safe">✓ Le PDF est transmis : vous pouvez fermer l’application.</p>` : ""}${job.error_message ? `<p class="ai-error">${h(job.error_message)}</p>` : ""}<div class="ai-actions">${active ? `<button class="btn" onclick="ResiAiJobs.cancel('${job.id}')">Annuler</button>` : ""}${job.status === "failed" ? `<button class="btn" onclick="ResiAiJobs.retry('${job.id}')">Réessayer</button>` : ""}</div></article>`;
  }
  function renderJobs() {
  ...
  async function open(currentUser) {
    ...
    const form = document.getElementById("aiCreate"), advanced = document.createElement("details"), advancedSummary = document.createElement("summary");
    advanced.className = "ai-advanced"; advancedSummary.textContent = "Options facultatives"; advanced.append(advancedSummary);
    [form.querySelector("[name=title]")?.closest("label"), form.querySelector("[name=specialty]")?.closest("label"), form.querySelector("[name=remember]")?.closest("label")].filter(Boolean).forEach((field) => advanced.append(field));
    ...
    poller = setInterval(refresh, 10000); timer = setInterval(tick, 1000);
  }
  ...
  root.ResiAiJobs = { open, filter: renderLibrary, cancel: async (id) => { try { await call("cancel", { jobId: id }); await refresh(); } catch (e) { alert(frenchError(e)); } }, retry: async (id) => { const apiKey = prompt("Clé API Gemini pour la nouvelle tentative"); if (apiKey) { try { await call("retry", { jobId: id, apiKey }); await refresh(); } catch (e) { alert(frenchError(e)); } } }, rename: async (id) => { const title = prompt("Nouveau nom"); if (title?.trim()) { try { await call("rename", { jobId: id, title: title.trim() }); await refresh(); } catch (e) { alert(frenchError(e)); } } }, remove: async (id) => { if (confirm("Supprimer définitivement cette épreuve ?")) { try { await call("delete", { jobId: id }); await refresh(); } catch (e) { alert(frenchError(e)); } } }, solve: (id) => { const j = jobs.find((x) => x.id === id), list = quiz(j).questions || []; state.session = { item: { id, title: j.title }, questions: list.map((q, i) => ({ id: q.id || `ia-${i}`, question: q.question || q.stem, options: Object.entries(q.options || {}).map(([key, text]) => ({ id: key, text, correct: (q.correct_answers || []).includes(key) })), explanation: q.explanation })), index: 0, selected: new Set(), checked: false, score: 0, origin: "ai" }; renderQuestion(); go("quizRunner"); }, pdf: async (id) => { const j = jobs.find((x) => x.id === id); try { const { url } = await call("download", { jobId: id }); location.href = url; } catch (_) { localPdf([j]); } }, json: (id) => { const j = jobs.find((x) => x.id === id); downloadBlob(new Blob([JSON.stringify(quiz(j), null, 2)], { type: "application/json" }), `${j.title}.json`); }, pack: () => { const ids = [...document.querySelectorAll(".ai-pack:checked")].map((n) => n.value), selected = jobs.filter((j) => ids.includes(j.id)); if (selected.length) localPdf(selected); else alert("Sélectionnez au moins une épreuve."); }, importJson: async (input) => { try { const body = JSON.parse(await input.files[0].text()); await call("import", { quiz: body }); await refresh(); } catch (e) { alert(frenchError(e)); } input.value = ""; } };
