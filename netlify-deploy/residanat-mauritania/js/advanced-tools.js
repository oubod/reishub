...
async function renderAiView(user) {
  ...
  $("ai").innerHTML = `<div class="ai-page"><section class="tool-heading"><p class="kicker">Quiz IA · Gemini 2.5 Flash</p><h2>Créer 30 QCM depuis un PDF</h2><p>Importez un document, vérifiez les conditions d’envoi, puis lancez une seule génération. Le PDF source n’est pas conservé.</p></section><div class="ai-layout"><section class="tool-card ai-generator"><h2>Nouvelle série</h2><p>10 questions faciles, 12 intermédiaires et 8 difficiles, avec plusieurs réponses possibles.</p><label class="field"><span>Clé API Gemini</span><input id="geminiKey" type="password" value="${esc(savedKey)}" autocomplete="off" placeholder="Collez votre clé API" aria-describedby="geminiKeyHelp"></label><p id="geminiKeyHelp" class="security-note">La clé reste dans le stockage local de cet appareil.</p><label class="field"><span>Document PDF · 20 Mo maximum</span><input id="geminiPdf" type="file" accept="application/pdf,.pdf"></label><label class="check"><input id="geminiConsent" type="checkbox"><span>J’accepte l’envoi de ce PDF à Google. L’utilisation peut être facturée et le contenu médical généré doit être vérifié.</span></label><div id="aiStatus" aria-live="polite"></div><button id="generateAiButton" class="btn primary wide" type="button" onclick="generateAiQuiz()">${icon("auto_awesome")}Générer 30 QCM</button><details class="ai-help" open><summary>Comment obtenir une clé API Gemini ?</summary><ol><li>Ouvrez <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer">Google AI Studio</a>.</li><li>Connectez-vous avec votre compte Google.</li><li>Choisissez <strong>Créer une clé API</strong>.</li><li>Copiez la clé et collez-la dans le champ ci-dessus. Ne la partagez jamais.</li></ol></details><p class="security-note">Une clé conservée dans le navigateur est moins sûre qu’un secret géré par serveur. Vous pouvez la supprimer à tout moment depuis le stockage du navigateur.</p></section><section class="ai-saved"><div class="ai-saved-header"><h2>Mes séries locales</h2><p>${sets.length} série${sets.length === 1 ? "" : "s"} sur cet appareil.</p></div><div class="saved-list">${sets.length ? sets.map(aiSetCard).join("") : `<div class="empty-card">Aucune série générée. Votre première série apparaîtra ici.</div>`}</div></section></div></div>`;
}
